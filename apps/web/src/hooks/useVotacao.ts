...
export function useVotacao() {
  ...
  const registrarPresenca = useCallback(
    async (
      assembleiaId: string,
      tipo: 'presencial' | 'online' | 'procuracao' | 'voto_antecipado' = 'online',
  ...
  const getMinhaPresenca = useCallback(
    async (assembleiaId: string): Promise<Presenca | null> => {
      try {
  ...
  const getResultadoPauta = useCallback(
    async (pautaId: string) => {
      try {
        const { data, error: fetchError } = await supabase
          .from('v_pauta_resultado')
          .select('*')
          .eq('pauta_id', pautaId)
          .single();
  ...
  const moderarComentario = useCallback(
  ...
  // Realtime votos
  const subscribeToVotos = useCallback(
    (pautaId: string, onUpdate: (resultado: unknown) => void) => {
      const channel = supabase
        .channel(`votos-${pautaId}`)
        .on(
          'postgres_changes',
          {
            event: 'INSERT',
